...
func _connect_enemies() -> void:
	call_deferred("_connect_enemies_retry")

func _connect_enemies_retry() -> void:
	var attempts := 0
	while attempts < 60:  # up to ~1 second at 60fps
...
func _on_player_hit() -> void:
	Input.set_mouse_mode(Input.MOUSE_MODE_VISIBLE)
	LoadingScreen.load_scene("res://Scenes/death_screen.tscn")
